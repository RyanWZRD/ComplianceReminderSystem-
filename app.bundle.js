...
${suffix}`;
  ...
  function getSupabaseClient() {
    if (!isSupabaseConfigured()) {
      throw new Error(
        "Supabase is not configured. Copy js/data/supabase-env.example.js to js/data/supabase-env.js, set SUPABASE_URL and SUPABASE_ANON_KEY in .env, then run: npm run sync-env"
      );
    }
    if (!client) {
  ...
  var AUTH_MODE = readAuthFromLocation() ?? (typeof process !== "undefined" && process.env?.AUTH_MODE === "supabase" ? "supabase" : typeof process !== "undefined" && process.env?.AUTH_MODE === "supabase-preview" ? "supabase-preview" : "local");
  ...
  function isValidRole(role) {
    return typeof role === "string" && VALID_ROLES.includes(
      /** @type {UserRole} */
      role
    );
  }
  function mapProfileRowToSessionUser(row) {
  ...
  async function fetchProfileForUser(supabase, userId) {
    const { data, error } = await supabase.from("profiles").select("id, organisation_id, display_name, role").eq("id", userId).maybeSingle();
    if (error) {
    ...
    try {
      ...
      const message = profileError instanceof Error ? profileError.message : "Invalid profile data.";
  ...
  async function restoreSessionFromClient() {
    if (!isSupabaseConfigured()) {
  ...
  function subscribeToAuthChanges(handler) {
    if (!isSupabaseConfigured()) {
  ...
  async function signInWithPassword2(email, password) {
  ...
  function initLoginPanel({ onAuthenticated }) {
    ...
    const form2 = document.getElementById("auth-login-form");
    if (form2) {
      form2.addEventListener("submit", handleLoginSubmit);
    }
  ...
  function showLoginScreen() {
    ...
    if (!screen) {
      ...
      if (bootError) {
        bootError.textContent = "Cloud sign-in UI is missing from the page. Ensure index.html includes #auth-login-screen, then hard-refresh (Ctrl+Shift+R).";
  ...
  function updateAuthChromeForCloud() {
    const signOutBtn = document.getElementById("sign-out-btn");
  ...
  async function handleLoginSubmit(event) {
    ...
    const result = await signInWithPassword2(email, password);
    if (submitBtn instanceof HTMLButtonElement) {
  ...
  function buildPeopleTree(peopleRows, recordRows, historyRows, evidenceRows, actionRows) {
    ...
    const recordsByPerson = /* @__PURE__ */ new Map();
  ...
  function mapComplianceRecord(record, historyByRecord, evidenceByRecord, actionsByRecord) {
    const history = (historyByRecord.get(record.id) || []).slice().sort((a, b) => a.created_at.localeCompare(b.created_at)).map(mapHistoryEntry);
  ...
  function mapDeletedSnapshots(rows) {
    return rows.map((row) => ({
  ...
  var LocalComplianceStore = class {
    ...
    normalizeHistoryEntry(entry) {
      const normalized = {
        id: typeof entry.id === "number" || typeof entry.id === "string" ? entry.id : this.nextHistoryEntryId++,
    ...
    normalizeEvidenceItem(item) {
      return {
        id: typeof item.id === "number" || typeof item.id === "string" ? item.id : this.nextEvidenceId++,
    ...
    normalizeActionItem(item) {
      ...
      return {
        id: typeof item.id === "number" || typeof item.id === "string" ? item.id : this.nextActionId++,
  ...
  // js/data/cloud-store.js
  var READ_ONLY_MESSAGE = "Cloud compliance store is read-only in this release. Writes are not enabled yet.";
  var CloudComplianceStore = class extends LocalComplianceStore {
    get backend() {
      return "cloud";
    }
    save() {
    ...
    normalizeDeletedSnapshot(item) {
      ...
      return {
        deletedAt: typeof item.deletedAt === "string" ? item.deletedAt : (/* @__PURE__ */ new Date()).toISOString(),
  ...
  async function fetchComplianceData(organisationId) {
    ...
    const [peopleResult, recordsResult, historyResult, evidenceResult, actionsResult, deletedResult] = await Promise.all([
      ...
      supabase.from("deleted_record_snapshots").select("id, deleted_at, person_name, person_role, record_snapshot").eq("organisation_id", organisationId).order("deleted_at", { ascending: false })
    ]);
    const firstError = peopleResult.error || recordsResult.error || historyResult.error || evidenceResult.error || actionsResult.error || deletedResult.error;
    if (firstError) {
  ...
  var CloudSettingsStore = class {
    ...
    setSettings() {
      throw new Error(READ_ONLY_MESSAGE2);
    }
    getSettings() {
    ...
    async load() {
      ...
      try {
        const supabase = getSupabaseClient();
        const { data, error } = await supabase.from("reminder_settings").select("days_30, days_14, days_7, hide_sent_reminders").eq("organisation_id", organisationId).maybeSingle();
        if (error) {
...
Your current data will be overwritten. Continue?`
  ...
  async function bootApp() {
    ...
    try {
      ...
      console.error(error);
      showBootError(
        error instanceof Error ? error.message : "The app failed to start while wiring controls. Check the browser console (F12)."
      );
      return;
    }
    if (isCloudMode()) {
      ...
      try {
        ...
        console.error(error);
        showBootError(
          error instanceof Error ? error.message : "Cloud authentication failed to initialize. Check the browser console (F12)."
        );
        showLoginScreen();
        return;
      }
      ...
      try {
        ...
        console.error(error);
        showBootError(
          error instanceof Error ? error.message : "The app failed to load cloud data. Check the browser console (F12)."
        );
      }
      return;
    }
    try {
      ...
      console.error(error);
      showBootError(
        error instanceof Error ? error.message : "The app failed to start. Check the browser console (F12)."
      );
    }
  }
  function startApp() {
