-- RC-001: allow in_progress -> completed (Phase 3 in-progress workflow)
...
begin
  ...
  if p_target_status = 'completed' and v_action.status not in ('open', 'in_progress') then
    return jsonb_build_object(
      'status',
      'invalid_transition',
      'reason',
      'only_open_or_in_progress_to_completed'
    );
  end if;
...
comment on function public.set_action_status(uuid, text) is
  'Transition action status to completed (from open/in_progress) or reopen to open with history (editor/admin).';
